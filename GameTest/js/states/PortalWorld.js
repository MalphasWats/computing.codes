function PortalWorld()
{
	...
	var monster_counts = [0, 20, 25, 400, 60]
	...
	this.setup = function()
	{
	    ...
	    if (depth == 1)
        {
            scientist.talk("Ouch! Hey, the potral! Noooooooo!")
        }
        ...
        var d = depth;
				if (d >= monster_counts.length) d = monster_counts.length -1;
        var monster_count =monster_counts[depth];// Math.floor(Math.random() * depth * 100)
        monsters = []
        ...
        if (depth > 0 && !scientist.resistor)
        {
            ...
            {
                scientist.talk("A Flux Resistor! With a few more parts I can open a portal hoem maybe...")
	...
	this.update = function()
	{
	    ...
	    jaws.update(monsters)
			var borked = depth == 2
	    viewport.centerAround(scientist, borked)
